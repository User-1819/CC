...
static void Classic_LevelFinalise(cc_uint8* data) {
	...
	if (!map1.blocks) {
		Chat_AddRaw("&4Failed to load map, try joining a different map");
		Chat_AddRaw("   &4Attempted to load map without a Blocks array");
	}
	if (map_volume != volume) {
		Chat_AddRaw("&4Failed to load map, try joining a different map");
		Chat_Add2(  "   &4Blocks array size (%i) does not match volume of map (%i)", &map_volume, &volume);
		FreeMapStates();
	}
